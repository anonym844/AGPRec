...
class DAGRec(nn.Module):
    def __init__(self,args,graph_g_u,graph,device):
        super(DAGRec, self).__init__()
        #parameter 
    ...
    def forward(self, graph, group_user):
        user_embed = self.g_u_forward(group_user)
    ...
    def aspect_regular(self):
        ...
        if self.isReg:
            ...
            for i in range(len(self.aspects)):
                for j in range(i + 1, len(self.aspects)):
    ...
    def g_u_forward(self,group_user):
        ...
        #Apply softmax
        #new
        embed_member_agg = torch.zeros(self.num_groups, len(self.aspects), self.embedding_dim).to(group_user.device)
